...
eval = \case
  ...
  T.TyApp ty tys tys' -> do
    ...
    forest <- zipWithM deduce holedArgs args
    let realArgs = bool
          {- Need deduction -}
          (catMaybes $ concat $ levels $ Node Nothing forest)
          {- No need deduction -}
          args
          (null holedArgs)
  ...
  T.TyCast ty ty'     -> do
    ...
    ty' <- eval ty'
    tys <- gets V.types
    printM $ "TyCast: " ++ show ty ++ " " ++ show ty' ++ " in " ++ show tys
    b <- ty <: ty'
    unless b $ throwError $ V.BadCast ty ty'
    return ty'
  T.TyBiCast ty ty'   -> do
    ty <- eval ty
    ty' <- eval ty'
    ...
    unless b $ throwError $ V.BadCast ty ty'
    return ty
  T.TyMacro _ _       -> throwError $ V.Unimplemented "Macro types"
    ...
    (V.TyVar i, t) -> do
      ...
      b <- fmap and
        ...
        $ \case
          Bot a -> eval a >>= (t <:)
          Top a -> eval a >>= (<: t)
      unless b $ throwError $ V.BadMatchedBorder t constrs
